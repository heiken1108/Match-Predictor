...
def plot_correlation_matrix(
    data: pd.DataFrame, figsize=(10, 8), cmap="Blues", annot=False
):
    plt.close("all")
    matrix = data.corr(method="pearson")
    plt.figure(figsize=figsize)
    sns.heatmap(
        matrix, cmap=cmap, annot=annot, linewidth=0.5
    )  # Added linewidth parameter
    plt.tight_layout()
    plt.show()
...
def remove_the_first_n_matches_in_a_season_for_each_team(data, n=5):
    ...
    print("number of matches getting removed: ", len(rows_to_remove))
